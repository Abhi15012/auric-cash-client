...
export default function QuickContact({ title }: { title: string }) {
  ...
  const defaultIntent =
    typeof window !== "undefined"
      ? window.location.href.includes("sell-gold")
        ? "sell"
        : window.location.href.includes("release-gold")
        ? "release"
        : window.location.href.includes("loan-gold")
        ? "sell-pledged"
        : null
      : null;
  ...
  const submitUserContact = async (
    ...
    // Ensure the returned value is always an object with a message property
